    ...
    R.keys(allDeps)
      .map(async (alias) => {
        if (allDeps[alias].startsWith('link:')) {
          ...
          const pkg = await readPackageFromDir(packageDir)
          await symlinkDirectRootDependency({
            alias,
            destModulesDir: opts.importerModulesDir,
            name: pkg.name,
            packageDir: path.join(opts.prefix, allDeps[alias].substr(5)),
            prefix: opts.prefix,
            saveType: isDev && 'devDependencies' || isOptional && 'optionalDependencies' || 'dependencies',
            version: pkg.version,
          })
          return
        }
