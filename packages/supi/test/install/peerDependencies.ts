...
test('peer dependency is grouped with dependent when the peer is a top dependency but an external shrinkwrap is used', async (t: tape.Test) => {
  ...
  t.deepEqual(shr['importers']['project'], { // tslint:disable-line
    dependencies: {
      'ajv': '4.10.4',
      'ajv-keywords': '/ajv-keywords/1.5.0_ajv@4.10.4',
    },
    specifiers: {
      'ajv': '^4.10.4',
      'ajv-keywords': '^1.5.0',
...
testOnly('peer dependency is grouped with dependent when the peer is a top dependency and external node_modules is used', async (t: tape.Test) => {
  const project = prepare(t)
  await mkdir('_')
  process.chdir('_')
  const shrinkwrapDirectory = path.resolve('..')
